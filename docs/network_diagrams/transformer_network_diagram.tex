...
\node[actor, below left=12mm and 13mm of split] (actorenc) {
  \textbf{actor token sequence}\\
  entities: Linear $26\to64$\\
  meta projection + learned CLS\\
  segment embeddings + padding mask
};

\node[critic, below right=12mm and 13mm of split] (criticenc) {
  \textbf{critic token sequence}\\
  entities: Linear $26\to64$\\
  meta projection + learned CLS\\
  segment embeddings + padding mask
};

\node[actor, below=of actorenc] (actortransformer) {
  \textbf{actor Transformer}\\
  two pre-LayerNorm blocks\\
  4-head bidirectional attention\\
  FFN width 256, zero dropout
};

\node[critic, below=of criticenc] (critictransformer) {
  \textbf{critic Transformer}\\
  two pre-LayerNorm blocks\\
  4-head bidirectional attention\\
  FFN width 256, zero dropout
};

\node[actor, below=of actortransformer] (actortrunk) {
  \textbf{actor outputs}\\
  CLS $\to$ MLP: $64\to128\to128$\\
  four contextual hand tokens $(B,4,64)$
};

\node[critic, below=of critictransformer] (critictrunk) {
  \textbf{critic output}\\
  CLS $\to$ MLP: $64\to128\to128$
};
...
\node[pointer, below left=13mm and 9mm of actortrunk] (candidates) {
  ...
  learned skip token\\
  + four contextual hand tokens $(B,5,64)$
};

\node[pointer, below right=13mm and 9mm of actortrunk] (query) {
  \textbf{state query}\\
  actor CLS state $\to$ linear projection\\
  $(B,128)\to(B,64)$
};
...
\node[actor, below=of scores] (condition) {
  ...
  actor CLS state\\
  + selected contextual hand token\\
  $(B,128)+(B,64)=(B,192)$
};
...
\node[note, below=of value] (shared) {
  \textbf{same supporting choices as DeepSets}\\
  disjoint actor-critic paths,\\
  action masking,\\
  learned temperatures,\\
  orthogonal initialization,\\
  linear and shared-network fallback switches
};

\draw[arrow] (obs) -- (split);
\draw[arrow] (split) -- (actorenc);
\draw[arrow] (split) -- (criticenc);
\draw[arrow] (actorenc) -- (actortransformer);
\draw[arrow] (criticenc) -- (critictransformer);
\draw[arrow] (actortransformer) -- (actortrunk);
\draw[arrow] (critictransformer) -- (critictrunk);
\draw[arrow] (critictrunk) -- (value);
\draw[arrow] (actortrunk) -- (query);
\draw[arrow] (actortrunk) -- (candidates);
\draw[arrow] (query) -- (scores);
\draw[arrow] (candidates) -- node[left, font=\sffamily\tiny] {hand tokens} (scores);
